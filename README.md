# 👋 Hi, I'm Mostafa Gholami (mst-ghi)

![GitHub followers](https://img.shields.io/github/followers/mst-ghi?style=social)
![Years of Experience](https://img.shields.io/badge/Experience-8%2B%20years-blue)

> Senior Full-Stack Developer | Frontend Focus | Scalable Systems & Monorepos

## 🚀 About Me

I'm a software engineer with **9+ years** of experience building scalable web applications. I specialize in **Node.js (NestJS)** and **Golang (Gin)** on the backend, paired with **React/Next.js** on the frontend — with a strong focus on clean code, feature-driven design, and maintainable systems.

For the past four years, I've been deeply focused on **NestJS + Next.js** stacks, **Turborepo monorepos**, shared component libraries, and **microservices architecture** (Kafka, Redis, socket programming). I'm now actively expanding into **Golang** for high-performance, concurrent backend services using the **Gin framework**.

I've built and maintained multi-tenant platforms, real-time dashboards, and high-performance aggregators used by thousands.

---

## 🛠️ Tech Stack

### Backend
![NestJS](https://img.shields.io/badge/-NestJS-E0234E?logo=nestjs&logoColor=white)
![Node.js](https://img.shields.io/badge/-Node.js-339933?logo=node.js&logoColor=white)
![Go](https://img.shields.io/badge/-Go-00ADD8?logo=go&logoColor=white)
![Gin](https://img.shields.io/badge/-Gin-00ADD8?logo=go&logoColor=white)
![PHP](https://img.shields.io/badge/-PHP-777BB4?logo=php&logoColor=white)
![Laravel](https://img.shields.io/badge/-Laravel-FF2D20?logo=laravel&logoColor=white)
![Kafka](https://img.shields.io/badge/-Kafka-231F20?logo=apache-kafka&logoColor=white)
![Redis](https://img.shields.io/badge/-Redis-DC382D?logo=redis&logoColor=white)
![TypeORM](https://img.shields.io/badge/-TypeORM-262627?logo=typeorm&logoColor=white)
![Prisma](https://img.shields.io/badge/-Prisma-2D3748?logo=prisma&logoColor=white)

### Frontend
![React](https://img.shields.io/badge/-React-61DAFB?logo=react&logoColor=black)
![Next.js](https://img.shields.io/badge/-Next.js-000000?logo=next.js&logoColor=white)
![Vue](https://img.shields.io/badge/-Vue-4FC08D?logo=vue.js&logoColor=white)
![Nuxt](https://img.shields.io/badge/-Nuxt-00DC82?logo=nuxt.js&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/-Tailwind-06B6D4?logo=tailwindcss&logoColor=white)
![MUI](https://img.shields.io/badge/-MUI-007FFF?logo=mui&logoColor=white)
![Mantine](https://img.shields.io/badge/-Mantine-339AF0?logo=mantine&logoColor=white)
![Ant Design](https://img.shields.io/badge/-Ant%20Design-0170FE?logo=antdesign&logoColor=white)
![Zustand](https://img.shields.io/badge/-Zustand-443E38?logo=zustand&logoColor=white)

### Architecture & Tools
![Turborepo](https://img.shields.io/badge/-Turborepo-EF4444?logo=turborepo&logoColor=white)
![Monorepo](https://img.shields.io/badge/-Monorepo-3E6B6B)
![Microservices](https://img.shields.io/badge/-Microservices-FF6B6B)
![Docker](https://img.shields.io/badge/-Docker-2496ED?logo=docker&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/-GitHub%20Actions-2088FF?logo=github-actions&logoColor=white)

---

## 📌 Featured Projects

| Project | Tech Stack |
|---------|------------|
| [**AllinGo**](https://allingo.vn/) – Aggregator Platform | Next.js, React (Vite), MUI, Tailwind, Zustand |
| [**AvidBizz**](https://avidbizz.com/) – Multi-tenant Business Platform | NestJS, Next.js, Turborepo, Mantine, Kafka |
| [**Digital Brand Boards**](https://alert.ir/) – IoT Billboard System | NestJS, Lumen, Kafka, Microservices (8 services) |
| [**Khanesarmaye ERP**](https://erp.khanesarmaye.com) – Analytics Dashboard | Next.js, React Query, Ant Design |

> 🔗 **More projects** – see my pinned repos below or check my [full portfolio](https://github.com/mst-ghi?tab=repositories)

---

## 📫 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mst-ghi/)
[![Telegram](https://img.shields.io/badge/Telegram-26A5C2?logo=telegram&logoColor=white)](https://t.me/@mostafa_ghi)
[![Email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:mostafagholamidev@gmail.com) <!-- Replace with real email -->

---

## 🧠 What I'm Currently Working On

- Building a **Turborepo-based monorepo** for a ride/delivery aggregator (AllinGo)
- Exploring **Golang + Gin** for high-performance microservices
- Writing shared UI component libraries with **Atomic Design** and **Storybook**

---

⭐ **Fun fact:** I've built systems that handle real-time socket connections for thousands of concurrent users — and I still get excited about a well-designed `useQuery` hook.
